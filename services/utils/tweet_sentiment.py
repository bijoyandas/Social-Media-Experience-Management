...
def sentiment_for_tweets(query, limit, country=None, area=500):
    with open("utils/models/model.pkl", "rb") as f:
        model = pickle.load(f)
    with open("utils/models/vectorizer.pkl", "rb") as f:
    ...
    for t in list_tweets:
        ...
        tweet_sentiment_dict['sentiment'] = str(model.predict_proba(vectorizer.transform([tweet]))[0][1])
